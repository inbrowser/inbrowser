...
export const createIntegral = factory('integrate',['parse', 'simplify','typed', 'ConstantNode', 'FunctionNode', 'OperatorNode', 'SymbolNode'], function ({ parse, simplify, typed, ConstantNode, FunctionNode, OperatorNode, SymbolNode }) {
     ...
     */
    function IntegrationContext(variable: any, options: any) {
     ...
     */
    IntegrationContext.prototype.isConstant = function(expr: any) {
     ...
     */
    IntegrationContext.prototype.printDebug = function(text: any) {
    ...
    function preprocessIntegrand(expr, context) {
        ...
        function reduceFunctions(expr: any) {
            return helper(expr);

            function helper(expr: any) {
        ...
        function removeDivision(expr: any) {
            return expr.transform(function(node: any) {
     ...
     */
    function getNumericValue(expr: any) {
        var simplified = simplifyConstant(expr);
        return toNumber(simplified);

        function toNumber(expr: any) : any {
    ...
    integral.rules = [
        ...
        function(expr: any, context: any, subIntegral: any) {
            // @ts-ignore
        ...
        function(expr: any, context: any, subIntegral: any) {
            if(expr.type === "OperatorNode" && (expr.op === '+' || expr.op === '-')) {
                var childInts = expr.args.map(function(expr: any) {
                    return subIntegral(expr, context, "sum rule");
                });

                if(childInts.every(function(n: any) { return n; })) {
        ...
        function(expr : any, context : any, subIntegral : any) {
            ...
            function getFactors(expr: any) : any {
                if(expr.type === "OperatorNode" && expr.op === "*") {
                    return expr.args.reduce(function(factors: any, expr: any) {
        ...
        function(expr: any, context: any, subIntegral: any) {
            if(expr.type === "OperatorNode" && expr.op === '*') {
                var constantFactors : any = [];
                var nonConstantFactors : any = [];
                expr.args.forEach(function(expr : any) {
                    if(context.isConstant(expr)) {
                ...
                // @ts-ignore
                function factorsToNode(factors: any) : any {
                    if(factors.length === 1) {
                        return factors[0];
                    } else {
        ...
        function(expr: any, context: any, subIntegral: any) {
            if(expr.type === "OperatorNode" && expr.op === '*') {
                ...
                var reducedArgs = argsAsPower.reduce(function(acc: any, exprPower: any) {
            ...
            function getExprInPowerForm(expr: any) {
                if(expr.type === "OperatorNode" && expr.op === '^') {
                    return {
                        base: expr.args[0],
                        power: expr.args[1]
            ...
            function powerFactorsToNode(factors: any) {
                if(factors.length === 1) {
                ...
                function powerToNode(powerExpr: any) {
        ...
        function(expr: any, context: any, subIntegral: any) {
            if(expr.type === 'OperatorNode' && expr.op === '^') {
                ...
                if(base.type === 'OperatorNode' && base.op === '*') {
                    return subIntegral(new OperatorNode('*', 'multiply', base.args.map(function(baseChild: any) {
        ...
        function(expr: any, context: any, subIntegral: any) {
            if(expr.type === "OperatorNode" && expr.op === '*') {
                var sumNode = null;
                var otherFactors: any = null;
                for(var i = 0; i < expr.args.length; i++) {
                    if(expr.args[i].type === "OperatorNode" && (expr.args[i].op === '+' || expr.args[i].op === '-'))  {
                        sumNode = expr.args[i];
                        otherFactors = expr.args.filter(function(expr: any, index: any) { return index !== i; });
                        break;
                    }
                }

                if(sumNode !== null) {
                    var newTerms = sumNode.args.map(function(term: any) {
        ...
        function(expr: any, context: any, subIntegral: any) {
            ...
            if(uniqueParent !== null && uniqueParent.type === "OperatorNode") {
                if(uniqueParent.op === '+' || uniqueParent.op === '-') {
                    if(uniqueParent.args.length === 1) {
                        // unary + or -
                        createIntegralWrapper = function(int: any) {
            ...
            function replaceNodeInTree(expr: any, node: any, replacement: any) {
                return replaceHelper(expr);

                function replaceHelper(curNode: any) {
                    return node.equals(curNode) ? replacement : curNode.map(replaceHelper);
                }
            }

            function getParentOfUniqueVariable(expr: any) {
                return helper(expr, null);

                function helper(expr: any, parent: any) : any {
                    if(context.isConstant(expr)) {
                        return null;
                    } else if(expr.type === "SymbolNode" && expr.name === context.variable.name) {
                        return parent;
                    } else {
                        var nonConstantChildren : Array<any> = [];
                        expr.forEach(function(child: any) {
        ...
        function(expr: any, context: any, subIntegral: any) {
            ...
            //
            function tryMultiplyOut(expr: any) {
     ...
     */
    function _integral(expr: any, context: any, lastRuleComment: any) {
