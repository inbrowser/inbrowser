...
describe('first_simple_derivative', function() {
	...
	describe('undefined or null fx', function() {
		[null, undefined].forEach(function(fx) {
			it(fx + ' fx', async function () {
				const variable = 'x';
	...
	describe('undefined or incorrect variable', function() {
		it('undefined variable', async function() {
			...
			let json = first_simple_derivative(fx,variable);
			assert.equal(json.result,null);
			assert.equal(json.error, "The parameter \"undefined\" is invalid. Expected {variable_name: value}.");
		});
		it('null variable', async function() {
			...
			let json = first_simple_derivative(fx, variable);
			assert.equal(json.result,null);
			assert.equal(json.error, "The parameter \"null\" is invalid. Expected {variable_name: value}.");
		});
...
describe('integral tests', function() {
	describe('undefined or null fx', function() {
		[null, undefined].forEach(function(fx) {
			it(fx + ' fx', async function () {
				const variable = 'x';
				let json = compute_integral(fx, variable);
				assert.equal(json.result, null);
				assert.equal(json.error, CalculusErrorMessages.InvalidFunction);
			});
		});
	...
	describe('undefined or incorrect variable', function() {
		[null, undefined].forEach(function(variable) {
			it(variable + ' variable', async function () {
				const fx = '10y+4x^2';
				let json = compute_integral(fx, variable);
				assert.equal(json.result, null);
				assert.equal(json.error, "The parameter \""+variable+"\" is invalid. Expected {variable_name: value}.");
			});
		});
	});
